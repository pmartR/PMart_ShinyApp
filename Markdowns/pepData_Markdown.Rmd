...
pepData <- readRDS(file.path(rootdir, "pepData_filter.RDS"))
pmart_inputs <- readRDS(file.path(rootdir, "pmart_inputs_filter.RDS"))
```
...
The following filters were applied with these parameters: 
...
getAppliedFilter <- function(addObject) {
  ifelse(addObject[1] == 1, "Applied", "")
}


`Filter Type` <- c(rep("Biomolecule", 4), rep("Sample", 2))
#`Filter Applied` <- 
```
