...
`ifelse(length(pepDataAttr$filters) == 0, "", paste("A total of", length(pepDataAttr$filters), "filters were applied. See the table below for a description of the order."))`
...
if (length(pepDataAttr$filters) != 0) {
  ...
  FilterTable <- do.call(rbind, lapply(pepDataAttr$filters[1:2], function(filter) {
    ...
    if (filter$type == "moleculeFilt") {
      return(c("Filter" = "Molecule Filter", "Type" = "Biomolecule", "Parameters" = 
               ...
               "Summary" = paste0(filter$filtered %>% length(), " biomolecules were filtered.")))
    } else
    ...
    if (filter$type == "proteomicsFilt") {
      return(c("Filter" = "Proteomics Filter", "Type" = "Biomolecule", "Parameters" = 
               ...
               "Summary" = paste0(filter$filtered$e_data_remove %>% length(), " biomolecules were filtered and ",
                                  filter$filtered$e_meta_remove %>% length(), " proteins were filtered.")))
    } else
      
    # CV Filter
    if (filter$type == "cvFilter") {
