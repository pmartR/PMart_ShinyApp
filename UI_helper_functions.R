...
apply_style_UI <- function(pagename, two_lipids, two_plots, flip_button = FALSE){
    if(flip_button){
      fbtn <- div(checkboxGroupButtons(paste0(pagename, '_flip_axes'), '', choices = list('Flip Axes' = TRUE),
    ...
    if(two_lipids){
      tagList(
        ...
        div(bsButton(paste0(pagename, '_apply_style_plot_1'), 'first dataset'), 
      ...
      tagList(
        tags$b('Update style to the plots for the:'),
        div(bsButton(paste0(pagename, '_apply_style_plot_1'), 'top/left plot'), 
