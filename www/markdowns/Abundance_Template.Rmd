...
if ("Molecule Filter" %in% FilterTable$Filter) {
  ...
  # Plot the filter 
  plot(molecule_filter(omicData, use_groups = filterInfo$method$use_groups, use_batch = filterInfo$method$use_batch),
       min_num = as.numeric(filterInfo$threshold))
...
```{r Proteomics Filter, echo = FALSE}
if ("Proteomics Filter" %in% FilterTable$Filter) {
  ...
  # Plot the filter 
  plot(proteomics_filter(omicData), min_num_peps = as.numeric(filterInfo$threshold$min_num_peps))

}
```

`r if ("Proteomics Filter" %in% FilterTable$Filter) paste0("**Code:**\n", proFilt_code)`
...
if ("CV Filter" %in% FilterTable$Filter) {
  ...
  # Plot the filter
  plot(cv_filter(omicData, use_groups = filterInfo$method$use_groups), cv_threshold = as.numeric(filterInfo$threshold))
  
}
```

`r if ("CV Filter" %in% FilterTable$Filter) paste0("**Code:**\n", cvFilt_code)`
...
if ("imd-ANOVA Filter" %in% FilterTable$Filter) {
  ...
  # Plot the filter
  plot(imdanova_filter(omicData), min_nonmiss_anova = as.numeric(filterInfo$threshold$min_nonmiss_anova),
       min_nonmiss_gtest = as.numeric(filterInfo$threshold$min_nonmiss_gtest))
  
}
```

`r if ("imd-ANOVA Filter" %in% FilterTable$Filter) paste0("**Code:**\n", imdFilt_code)`
...
if ("rMD Filter" %in% FilterTable$Filter) {
  ...
  # Plot the filter
  plot(rmd_filter(omicData), pvalue_threshold = as.numeric(filterInfo$threshold))
  
}
```

`r if ("rMD Filter" %in% FilterTable$Filter) {"Below are the summary statistics of each sample that was removed. If this section is blank, no samples were removed."}`

```{r rMD Filter Samples, echo = FALSE}
if ("rMD Filter" %in% FilterTable$Filter) {
  
  # Pull filter information
  filterInfo <- omicDataAttr$filters[[which(FilterTypes == "rmdFilt")[1]]]
  
  # Plot all samples removed
  lapply(filterInfo$filtered, function(samp) {
    #plot(rmd_filter(omicData), sampleID = samp) # Enable this once the error is fixed 
    paste("The sample", samp, "was removed.")
  })
...
`r if ("rMD Filter" %in% FilterTable$Filter) paste0("**Code:**\n", rmdFilt_code)`
...
`r if (omicDataAttr$data_info$norm_info$is_normalized) {paste0("**Code**: ", norm_code)}`
...
if (is.null(omicStatsRoll) == FALSE) {
  ComparisonsText <- paste("The following groups were compared on the peptide level:", paste0(paste(omicStatsRollAttr$comparisons, sep = ","), 
...
if (is.null(omicStatsRoll) == FALSE) {
  ...
  # Generate the plot
  plot(omicStatsRoll, bw_theme = TRUE)

}
```

`r if (!is.null(omicStatsRoll)) {paste0('**Code**: ', pepStat_code)}`
...
`r if (!is.null(omicStatsRoll)) {paste0('**Code**: ', rollup_code)}`

`r if (!is.null(omicStats)) {"## Statistics \n\n"}`

```{r Statistics: Text, echo = FALSE}
if (is.null(omicStats) == FALSE) {
  ComparisonsText <- paste("The following groups were compared on the peptide level:", paste0(paste(omicStatsAttr$comparisons, sep = ","), 
    "."), "Reported below are the parameters used in the statistical analysis,",
    "followed by the number of siginificant biomolecules for each comparison.")
}
```
