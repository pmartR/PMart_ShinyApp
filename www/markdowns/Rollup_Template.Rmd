---
params:
  titleName: "Rollup Template File"
  pepData: ""
  proStats: ""
  pmart_inputs: ""
  spans_results: ""
...
Peptides **were** rolled up into proteins. Protein quantification was conducted with the **`r pmart_inputs$which_rollup`** method and was centered with the **`r pmart_inputs$which_combine_fn`** function. The quantile cutoff was restricted to **`r  pmart_inputs$qrollup_thresh`**.

`r if (pmart_inputs$ReportCode) {"### *Example Code*"}`

```{r, eval = FALSE, echo = pmart_inputs$ReportCode}
# This is example code for how to rollup peptides into proteins

# Note: There are many rollup parameters to consider. See ?protein_qaunt for more information
normalproData <- protein_quant(pepData = normalpepData, method = "rollup")
```
