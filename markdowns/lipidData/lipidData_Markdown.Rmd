...
rootdir <- "/Users/degn400/Desktop/Git_Repos/pmart_standalone/Markdowns/lipidData"
lipidData <- readRDS(file.path(rootdir, "lipidData.RDS"))
lipidDataAttr <- attributes(lipidData)
lipidStats <- readRDS(file.path(rootdir, "lipidStats.RDS"))
lipidStatsAttr <- attributes(lipidStats)
pmart_inputs <- readRDS(file.path(rootdir, "pmart_inputs_lipid.RDS"))

# Create sentence for scale data change
if (lipidDataAttr$data_info$data_scale_orig == lipidDataAttr$data_info$data_scale) {
  ScaleData <- paste0("The original scale of the data was ", lipidDataAttr$data_info$data_scale_orig,
    " and remained unchanged.")
} else {
  ScaleData <- paste0("The original scale of the data was ", lipidDataAttr$data_info$data_scale_orig,
    " and was changed to ", lipidDataAttr$data_info$data_scale, ".")
}
...
that designates unique molecules was marked as **"`r lipidDataAttr$cnames$edata_cname`"**. 
...
if (is.null(lipidDataAttr$cnames$emeta_cname) == FALSE) {
  BiomoleculeText <- paste("An associated biomolecule information file was also",
    ...
    "was designated as", paste0(lipidDataAttr$cnames$emeta_cname, "."))
}
```
...
designated as **"`r lipidDataAttr$cnames$fdata_cname`"**.
...
Filters **`r ifelse(length(lipidDataAttr$filters) == 0, "were not", "were")`** applied.
`r ifelse(length(lipidDataAttr$filters) == 0, "", paste("A total of", length(lipidDataAttr$filters), "filters were applied. See the table below for a descriptions of the filters and the order."))`
...
if ("Molecule Filter" %in% FilterTable$Filter) {
  MoleculeFilterText <- paste0("A molecule filter was applied to the data, which removes 
    biomolecule(s) (", paste0(lipidDataAttr$cnames$edata_cname, "s"), ") not having at least", 
...
if ("CV Filter" %in% FilterTable$Filter) {
  CVFilterText <- paste0("A coefficient of variation (CV) filter was applied to the data ",
    "which removes biomolecule(s) (", paste0(lipidDataAttr$cnames$edata_cname, "s"), ") with a CV greater than ",
...
if ("imd-ANOVA Filter" %in% FilterTable$Filter) {
  imdAnovFilterText <- paste0("An ANOVA filter can be applied to the data which removes biomolecule(s) (",
    paste0(lipidDataAttr$cnames$edata_cname, "s"), ") not having at least a minimum number ",
...
if ("rMD Filter" %in% FilterTable$Filter) {
  rMDFilterText <- paste0("A robust Mahalanobis distance (rMD) filter was applied to the",
    " data, removing sample(s) (", paste0(lipidDataAttr$cnames$fdata_cname, "s"), ") with an associated ",
...
Peptide data **`r ifelse(lipidDataAttr$data_info$norm_info$is_normalized, "was", "was not")`**
normalized.
