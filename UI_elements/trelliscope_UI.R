...
output$trelliscope_out <- renderTrelliscope({
  ...
  if (input$make_trelliscope == 0) {
    ...
    isolate({
      ...
      withProgress(
        message = "Making your display...", value = 1,
        revals$trelli <- trelliVis(
          omicsData = object, omicsStats = stats, panel_variable = get_edata_cname(objects$omicsData),
...
output$trelli_plot_type <- renderUI({
  ...
  choices <- c(
    "Abundance Boxplot" = "abundance_boxplot", "Relative Abundance" = "abundance_global",
