list(
  ...
  output$fdata_UI <- renderUI({
    if(two_lipids()){
      tagList(
        ...
        fluidRow(
          column(6,
                 div(id = 'js_file_fdata', style = 'overflow-x:hidden',
                     fileInput('file_fdata', label = NULL,
                               multiple = FALSE,
                               accept = c('text/csv',
          ...
          column(6,
                 div(id = 'js_file_fdata_2', style = 'overflow-x:hidden',
                     fileInput('file_fdata_2', label = NULL,
                               multiple = FALSE,
                               accept = c('text/csv',
                                          'text/comma-separated-values,text/plain',
                                          '.csv'))
  ...
  output$group_barplots <- renderUI({
    ...
    else div(id = 'sample_barplots_1', style = 'border-style:solid;border-width:1px;', plotOutput('group_barplots_1'))
  }),
  ## 
  ...
  output$customsampnames <- renderUI({
    ...
    if(input$customsampnames_opts == 'first_n'){
      ...
      max_elements = sapply(sample_names(), strsplit, input$delimiter) %>% sapply(length) %>% max()
      tagList(
  ...
  output$customsampnames_2 <- renderUI({
    ...
    if(input$customsampnames_opts == 'first_n'){
      ...
      max_elements = sapply(sample_names_2(), strsplit, input$delimiter_2) %>% sapply(length) %>% max()
      tagList(
  ...
  output$grouped_data_summary <- renderUI({
    ...
    wellPanel(
      tagList(
        ...
        else div(id = 'groups_summary_1', DTOutput('omicsData_groups_summary'))
      )
    )
  }),
  #
