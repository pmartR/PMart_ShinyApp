list(
  ...
  output$qc_plots <- renderUI({
    ...
    else div(id = 'qc_plots_1', style = 'border-style:solid;border-width:1px;', plotOutput('omicsData_plot'))
  }),
  ...
  output$omicsData_plot <- renderPlot({
    ...
    if(input$which_qc_plot == 'boxplots'){
      ...
      p <- plot(objects$omicsData, order_by = order_by, color_by = color_by,
              use_VizSampNames = use_VizSampNames,
              bw_theme = TRUE)
    }
    else if(input$which_qc_plot == 'missingval_bar'){
      p <- plot(missingval_result(objects$omicsData), type = input$missingval_type,
             use_VizSampNames = use_VizSampNames,
  ...
  output$omicsData_plot_2 <- renderPlot({
    ...
    if(input$which_qc_plot == 'boxplots'){
      ...
      p <- plot(objects$omicsData_2, order_by = order_by, color_by = color_by,
             bw_theme = TRUE,
             use_VizSampNames = use_VizSampNames)
    }
    else if(input$which_qc_plot == 'missingval_bar'){
      p <- plot(missingval_result(objects$omicsData_2), type = input$missingval_type,
             use_VizSampNames = use_VizSampNames,
             bw_theme = TRUE,
             palette = input$qc_colors)
    }
    else if(input$which_qc_plot == 'missingval_scatter'){
      p <- missingval_scatterplot(objects$omicsData_2, bw_theme = TRUE,
                             palette = input$qc_colors)
    }
