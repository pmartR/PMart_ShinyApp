...
rootdir <- "/Users/degn400/Desktop/Git_Repos/pmart_standalone/Markdowns/pepData"
pepData <- readRDS(file.path(rootdir, "pepData_Evan.RDS"))
...
if (pepDataAttr$data_info$data_scale_orig == pepDataAttr$data_info$data_scale) {
  ScaleData <- paste0("The original scale of the data was ", pepDataAttr$data_info$data_scale_orig,
    " and remained unchanged.")
} else {
  ScaleData <- paste0("The original scale of the data was ", pepDataAttr$data_info$data_scale_orig,
    " and was changed to ", pepDataAttr$data_info$data_scale, ".")
}
...
that designates unique molecules was marked as **"`r pepDataAttr$cnames$edata_cname`"**. 
...
if (is.null(pepDataAttr$cnames$emeta_cname) == FALSE) {
  BiomoleculeText <- glue::glue_col(paste("An associated biomolecule information file was also",
    "uploaded called {bold ", pmart_inputs$file_emeta$name, "} and the protein identifier column",
    "was designated as {bold ", paste0(pepDataAttr$cnames$emeta_cname, "}.")))
}
```
...
Filters **`r ifelse(length(pepDataAttr$filters) == 0, "were not", "were")`** applied.
`r ifelse(length(pepDataAttr$filters) == 0, "", paste("A total of", length(pepDataAttr$filters), "filters were applied. See the table below for a descriptions of the filters and the order."))`
...
if ("Molecule Filter" %in% FilterTable$Filter) {
  MoleculeFilterText <- paste0("A molecule filter was applied to the data, which removes 
    biomolecule(s) (", paste0(pepDataAttr$cnames$edata_cname, "s"), ") not having at least", 
...
if ("CV Filter" %in% FilterTable$Filter) {
  CVFilterText <- paste0("A coefficient of variation (CV) filter was applied to the data ",
    "which removes biomolecule(s) (", paste0(pepDataAttr$cnames$edata_cname, "s"), ") with a CV greater than ",
...
if ("imd-ANOVA Filter" %in% FilterTable$Filter) {
  imdAnovFilterText <- paste0("An ANOVA filter can be applied to the data which removes biomolecule(s) (",
    paste0(pepDataAttr$cnames$edata_cname, "s"), ") not having at least a minimum number ",
...
if ("Proteomics Filter" %in% FilterTable$Filter) {
  ProteomicsFilterText <- paste0("A degenerate peptide filter (Degenerate Peptides Removed)",
    ...
    "which identifies biomolecule(s) (", paste0(pepDataAttr$cnames$edata_cname, "s"), ") not belonging ",
...
if ("rMD Filter" %in% FilterTable$Filter) {
  rMDFilterText <- paste0("A robust Mahalanobis distance (rMD) filter was applied to the",
    " data, removing sample(s) (", paste0(pepDataAttr$cnames$fdata_cname, "s"), ") with an associated ",
...
The statistical analysis step **`r ifelse(is.null(pepStats), "was not", "was")`** run.
