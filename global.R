...
ttext_ <- list(
  ...
  REF_GROUP_INFO = "Reference normalization expects samples to belong to one of several groups, each having a single &quot;reference&quot; sample.  Indicate the column in the groups file that specifies such a grouping.",
  REF_SAMP_INDICATOR_COL_INFO = "Select which column in the groups file indicates whether or not a sample is a reference sample for its reference group. For example, this column might contain only 1s and 0s, with 1s indicating a reference sample, and would have only a single 1 for each reference group.",
  REF_SAMP_INDICATOR_SYMBOL_INFO = "Select what value indicates a reference sample.  In the column you selected as indicating reference samples, what value in that column indicates a reference sample.  For example, you might specify that &quot;1&quot; represents a reference sample.",
