...
observeEvent(c(input$top_page, input$spans_or_manual), {
  ...
  if(using_spans) updateCollapse(session, 'spans_submenu', open = 'use_spans')
  else updateCollapse(session, 'spans_submenu', open = 'choose_params')
  #
...
output$spans_ppp_rip <- renderUI({
  ppp_rip_text <- sprintf('<b>ppp_rip values (based off element-wise combination of ppp and rip):<b/><br>%s', 
                          revals$spans_params[['ppp_rip']] %>% lapply(function(el){sprintf('(%s, %s)', el[1], el[2])}) %>% paste(collapse = ' | '))
  HTML(ppp_rip_text)
})
...
observe({
  ...
  if(!is.null(revals$spans_params)){
    ...
    are_valid <- sapply(subset_fn, function(el){
      revals$spans_params[[el]] %>% unlist() %>% sapply(function(x){isTRUE(x >= 0 & x <= 1)}) %>% {length(.) > 0 & all(.)}
    })
      ...
      modalDialog({
        fluidRow(
          column(10, align = "center", offset = 1,
                 tags$h4(sprintf('Your data has been normalized using subset function: (%s) with parameters: (%s) and normalization function: (%s)', 
...
observeEvent(input$inspect_norm, {
  ...
  res_1 <- tryCatch({inspect_norm(objects$omicsData, subset_fn = input$subset_fn, norm_fn = input$norm_fn, params = params)
    },error = function(e){
  ...
  if(!is.null(objects$omicsData_2)){
    res_2 <- tryCatch({inspect_norm(objects$omicsData_2, subset_fn = input$subset_fn, norm_fn = input$norm_fn, params = params)
      }, error = function(e){
