...
observeEvent(input$add_molfilt, {
  ...
  else revals$warnings_filter$molecule_filter_1 <- revals$warnings_filter$molecule_filter2 <- objects$filters$molfilt <- objects$filters$molfilt_2 <- NULL
...
observeEvent(input$add_cvfilt, {
  ...
  else revals$warnings_filter$cv_filter1 <- revals$warnings_filter$cv_filter2 <- objects$filters$cvfilt <- objects$filters$cvfilt_2 <- NULL
...
observeEvent(input$add_profilt, {
  ...
  else revals$warnings_filter$profilt <- objects$filters$profilt <- NULL
...
observeEvent(input$add_imdanovafilt,{
  ...
  else revals$warnings_filter$imdanovafilt1 <- revals$warnings_filter$imdanovafilt2 <- objects$filters$imdanovafilt <- objects$filters$imdanovafilt_2 <- NULL
...
observeEvent(input$add_rmdfilt,{
  ...
  else revals$warnings_filter$rmdfilt1 <- revals$warnings_filter$rmdfilt2 <- objects$filters$rmdfilt <- objects$filters$rmdfilt_2 <- NULL
...
observeEvent(input$add_customfilt,{
  ...
  else revals$warnings_filter$customfilt1 <- revals$warnings_filter$customfilt2 <- objects$filters$customfilt <- objects$filters$customfilt_2 <- NULL
...
observeEvent(c(input$plot_profilt, input$min_num_peps, input$degen_peps), {
  ...
  plots$filter_mainplot <- tryCatch({plot(proteomics_filter(objects$uploaded_omicsData), min_num_peps = as.numeric(input$min_num_peps), bw_theme = TRUE)},
                                      error = function(e){
...
observeEvent(c(input$plot_molfilt, input$mol_min_num), {
  ...
  plots$filter_mainplot <- tryCatch({plot(molecule_filter(objects$uploaded_omicsData), min_num = input$mol_min_num, bw_theme = TRUE)},
                                      error = function(e){
  ...
  if(!is.null(objects$uploaded_omicsData_2)){
    plots$filter_mainplot_2 <- tryCatch({plot(molecule_filter(objects$uploaded_omicsData_2), min_num = input$mol_min_num, bw_theme = TRUE)},
                                        error = function(e){
...
observeEvent(c(input$plot_cvfilt, input$cv_threshold), {
  ...
  plots$filter_mainplot <- tryCatch({plot(cv_filter(objects$uploaded_omicsData), cv_threshold = input$cv_threshold, bw_theme = TRUE)},
                                      error = function(e){
  ...
  if(!is.null(objects$uploaded_omicsData_2)){
    plots$filter_mainplot_2 <- tryCatch({plot(cv_filter(objects$uploaded_omicsData_2), cv_threshold = input$cv_threshold, bw_theme = TRUE)},
                                        error = function(e){
...
observeEvent(c(input$plot_imdanovafilt, input$min_nonmiss_anova, input$min_nonmiss_gtest), {
  ...
  plots$filter_mainplot <- tryCatch({plot(imdanova_filter(objects$uploaded_omicsData), min_nonmiss_anova = input$min_nonmiss_anova, min_nonmiss_gtest = input$min_nonmiss_gtest, bw_theme = TRUE)},
                                      error = function(e){
  ...
  if(!is.null(objects$uploaded_omicsData_2)){
    plots$filter_mainplot_2 <- tryCatch({plot(imdanova_filter(objects$uploaded_omicsData_2), min_nonmiss_anova = input$min_nonmiss_anova, min_nonmiss_gtest = input$min_nonmiss_gtest, bw_theme = TRUE)},
                                        error = function(e){
...
observeEvent(input$filter_apply_style_plot_1,{
  ...
  theme <- theme(axis.title.x = element_text(size = input$filter_x_fontsize), 
  ...
  else plots$filter_mainplot <- plots$filter_mainplot + xlab(input$filter_xlab) + ylab(input$filter_ylab) + ggtitle(input$filter_title) + theme
})
...
observeEvent(input$filter_apply_style_plot_2,{
  ...
  else plots$filter_mainplot_2 <- plots$filter_mainplot_2 + xlab(input$filter_xlab) + ylab(input$filter_ylab) + ggtitle(input$filter_title) + theme
})
