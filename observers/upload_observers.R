...
observeEvent(c(input$file_edata, input$file_edata_2),{
  ...
  else cond = !is.null(input$file_edata)
  
  if(cond){
    updateCollapse(session, 'upload_collapse_left', open = c('columnids'), close = 'datselect')
  }
  else updateCollapse(session, 'upload_collapse_left', close = c('columnids'))
...
observe({
  ...
  if(two_lipids()){
    ...
    cond_idcol_edata = all(sapply(list(e_data(), e_data_2()), 
                                  function(df){
                                      isTRUE(input$id_col %in% colnames(df))
                                  }
                                )
                          )
    cond_idcol_emeta = all(sapply(list(revals$e_meta, revals$e_meta_2), 
                                  function(df){
...
observeEvent(input$makeobject, {
  ...
  objects$uploaded_omicsData <- objects$omicsData <- tryCatch({
    object_fn(e_data = e_data(), e_meta = revals$e_meta, f_data = f_data_upload(), 
  ...
  if(two_lipids()){
    objects$uploaded_omicsData_2 <- objects$omicsData_2 <- tryCatch({
      object_fn(e_data = e_data_2(), e_meta = revals$e_meta_2, f_data = f_data_upload_2(), 
  ...
  else objects$omicsData_2 <- objects$uploaded_omicsData_2 <- NULL
