...
inspect_norm <- function(omicsData, subset_fn, norm_fn, params){
  ...
  if(!is.null(attributes(omicsData)$group_DF)){
    ...
    loc_boxplot <- loc_df %>% ggplot() + geom_boxplot(aes(x=Group, y = VAL__, fill = Group)) + 
      ylab('Location Parameter Value') + ggtitle('Location parameter values for chosen normalization, by group')
    ...
    if(!is.null(params$scale)){
      ...
      scale_boxplot <- scale_df %>% ggplot() + geom_boxplot(aes(x=Group, y = VAL__, fill = Group)) + 
        ylab('Scale Parameter Value') + ggtitle('Scale parameter values for chosen normalization, by group')
    }
    else scale_boxplot <- NULL
  }
